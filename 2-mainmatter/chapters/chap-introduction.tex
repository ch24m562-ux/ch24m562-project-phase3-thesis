...
reinforcement-learning comparators, so that any observed advantage can be
attributed specifically to ordering rather than to dispatch quality or
forecasting alone:
...
\begin{description}
  ...
  policy, when dispatch architecture is held constant?
...
\begin{enumerate}
  ...
  operational constraints, alongside hierarchical decomposition baselines in
  ...
  a perfect-information Oracle-MPC reference --- across ten sites, ten
  independent training seeds, and four delivery-logistics scenarios of
  increasing severity, quantifying improvements in unserved energy, diesel
  consumption, and network uptime with paired statistical comparisons
  throughout.
\end{enumerate}
...
\begin{enumerate}
  ...
  feasibility enforcement via action masking, benchmarked against both a
  hierarchical reinforcement-learning decomposition (TrackB) and
  optimisation-based dispatch (MPC, Oracle-MPC) using a shared classical
  $(s,S)$ ordering policy --- a comparison that isolates the contribution of
  ordering from the contribution of dispatch quality, which is not present in
  ...
  ten independent training seeds and four delivery-logistics scenarios with
  paired seed-level statistical methodology throughout.

  \item A scenario-dependent empirical characterisation of when joint
  optimisation is beneficial: the paired ablation isolating replenishment
  learning shows a significant advantage under normal and severe logistics
  stress, no distinguishable advantage under moderate stress, and clarifies
  that action masking's primary benefit is robustness to logistics conditions
  outside the training distribution rather than deployment-time constraint
  enforcement. An exploratory extension investigating explicit delivery-delay
  tracking (RLInv-DA) is also reported, including its null result, as a
  genuine finding rather than an omitted one.
\end{enumerate}
...
\begin{itemize}
  \item The RL agent is trained under Normal and Delayed logistics only,
  with Monsoon and Extreme scenarios used purely as out-of-distribution
  evaluation stress tests. Its advantage over classical ordering is
...
Accordingly, the objective of this thesis is not only to evaluate whether
reinforcement learning improves operational performance, but also to
understand under which operating conditions jointly learning replenishment
and dispatch provides measurable benefit over conventional rule-based and
...
\begin{itemize}
  ...
  agent architecture, the full comparison-policy set (rule-based,
  reinforcement learning, and optimisation-based, including MPC and Oracle-MPC),
  the training pipeline, and the evaluation protocol.
  ...
  experimental results, including the main EENS comparison against both
  reinforcement-learning and optimisation-based baselines, the structured
  ablation addressing H1, H2, and~H3, robustness checks across reward
  design, tank capacity, delivery-distribution form, and stochastic grid
  outages, and a per-site performance breakdown across the ten evaluated
  sites.

  \item \textbf{Chapter~\ref{chap:conclusion}} discusses what the results
  mean and why the scenario-dependent findings occurred, summarises the
  principal contributions and limitations, and outlines future work
  directions including richer logistics modelling, fleet-level multi-agent
  extensions, and validation against empirical delivery data.
\end{itemize}
