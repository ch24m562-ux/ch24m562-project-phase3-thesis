...
As of 2025, mobile networks serve more than 5.6--5.8 billion unique
...
2--8\,kW continuously, making the Radio Access Network (RAN)
responsible for 60--80\% of a mobile operator's total electricity
demand \citep{americas2023_energy_efficiency}. Globally, mobile networks
already account for approximately 2--3\% of worldwide electricity
consumption, a figure projected to rise further as 5G sites increase per-site
power demand by 2--3$\times$ compared to 4G
...
rely on diesel generators (DGs) and battery banks.
...
demonstrated 20--65\% reductions in diesel consumption and CO\textsubscript{2}
...
1.2 billion subscribers and approximately
400{,}000--500{,}000 tower sites---the challenge is particularly
...
The problem addressed here is how to optimally manage energy resources
...
\begin{description}
  ...
  \item[H3 (Inventory observation).] Does removing the five inventory- and
...
\begin{enumerate}
  ...
  increasing severity. Quantify improvements in unserved energy, diesel
...
\begin{enumerate}
  ...
  feasibility enforcement via action masking. RLInv is benchmarked against
  both a dispatch-only RL baseline with externally managed replenishment
  (TrackB) and
  ...
  ordering from the contribution of dispatch quality and is not present in
  ...
  learning (H1) shows a significant advantage under normal and severe
  logistics stress and no distinguishable advantage under moderate stress.
  A separate ablation shows that masking prevents severe dispatch-policy
  degradation under stressed logistics when replenishment is externally
  managed; no measurable mean-EENS benefit is observed when ordering is
  already learned jointly. An exploratory extension investigating explicit delivery-delay
