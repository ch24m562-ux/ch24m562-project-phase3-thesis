...
As of 2025, mobile networks serve approximately 5.7 billion unique
...
\begin{description}
  ...
  \item[H2 (Action masking).] Does hard action masking improve reliability
  under stressed logistics, and does its effect depend on whether diesel
  replenishment is externally managed or learned jointly?
...
\begin{enumerate}
  ...
  (MPC and Oracle-MPC), and the controlled A6 ablation. The RLInv--A6
  comparison isolates the contribution of jointly learned replenishment
  while holding the dispatch architecture comparable, a controlled
  comparison not present in
  ...
  optimisation is beneficial: the H1 comparison favours RLInv under Normal
  and Extreme logistics, while no statistically distinguishable advantage
  is observed under Delayed or Monsoon conditions.
  
   \item A separate ablation shows that masking prevents severe dispatch-policy
  ...
  already learned jointly.

  \item An exploratory extension investigating explicit delivery-delay
  tracking (RLInv-DA) is also reported, including the finding that explicit
  delay awareness did not provide a consistent improvement over RLInv, as a
...
\begin{itemize}
  ...
  \item The stochastic diesel-delivery process is the principal synthetic
  time-varying overlay. Load, PV generation, and grid-availability inputs
