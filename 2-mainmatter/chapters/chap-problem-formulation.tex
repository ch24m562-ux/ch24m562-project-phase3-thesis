...
control problem becomes a stochastic mixed-integer dynamic program once diesel
ordering, generator switching, and energy dispatch decisions are coupled across
time. Solving such problems optimally requires accurate probabilistic models
of solar generation, load variability, grid outages, and fuel delivery
delays---all of which vary substantially across sites and seasons and are
difficult to specify in closed form. Reinforcement learning provides an
alternative approach by learning a policy directly from interaction with
historical system trajectories, enabling the controller to adapt to stochastic
conditions without repeatedly solving a large-scale optimisation problem at
every timestep. Furthermore, once trained, the learned policy generalises
across sites and operating scenarios, whereas a model-based optimisation must
be re-solved from scratch for each new instance.
...
day as a two-dimensional cyclic feature.

The observation consists of seven logical components, corresponding to nine
scalar inputs after expansion of the cyclic hour-of-day encoding.
...
\bottomrule
\end{tabular}
\caption{Data provenance of each state variable. $\Pipe_t$ is the only
component introduced synthetically through an explicit stochastic lead-time
model; the remaining components are either obtained directly from the
ITU/Zindi dataset or evolved from dataset-grounded initial conditions.}
...
\]
where $\mathcal{S} \subset \mathbb{R}^9$ is the state space defined in
...
ITU/Zindi dataset, providing an independent assessment of generalisation.

% ---------------------------------------------------------------
\section{Unified versus Hierarchical Decomposition}
...
The comparison between unified and hierarchical architectures allows the
following hypotheses to be tested.

\textbf{H1 (Coupling hypothesis).} Joint optimisation of diesel ordering and
energy dispatch by a single agent provides measurable reliability or cost
benefits relative to hierarchical decomposition. A reduction of at least 10\%
in Expected Energy Not Served (EENS) is considered the threshold for
operational significance.

\textbf{H2 (Action masking hypothesis).} Incorporating feasibility-aware
action masking during training improves learning stability and reduces
the likelihood of infeasible exploratory actions relative to vanilla
policy-gradient training without masking.
...
agent in which the inventory-related state variables $\Inv_t$ and $\Pipe_t$
are removed from the observation vector.
