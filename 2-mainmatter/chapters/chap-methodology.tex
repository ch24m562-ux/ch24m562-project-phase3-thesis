...
diesel stockout or battery deep-discharge causes
immediate operational constraint violations or loss of service. \texttt{MaskablePPO} from \texttt{sb3-contrib} instead sets
...
property of the policy distribution and avoids relying on penalty tuning
alone for hard feasibility constraints; a violation-backstop coefficient
($\mu$) is still retained (Section~\ref{sec:reward_design}).
...
ablation results there show that masking mainly prevents severe
dispatch-policy degradation under stressed logistics when replenishment
is externally managed, rather than acting primarily to eliminate
...
fitted to data, an approach consistent with reinforcement learning
studies in energy-management applications, where reward design is
subsequently validated through sensitivity analysis to demonstrate
robustness rather than claiming a unique optimal
weighting~\citep{hu2024performance}. Chapter~\ref{chap:results} reports a systematic
...
\node[label] at (3.4, -1.55) {Decomposed Control (TrackB)};
...
Oracle-MPC & $(s,S)$ rule       & MILP, $H{=}360$      & Perfect & Optimistic reference under perfect future information \\
TrackB  & $(s,S)$ rule          & Learned (PPO)        & --- & Joint vs.\ decomposed (H1, confounded) \\
A6      & $(s,S)$ rule          & Learned (PPO, Discrete(2)) & --- & Joint vs.\ decomposed (H1, cleaner) \\
