% filename: chap-methodology-v2.tex
% Phase 3 rewrite: restructured into 4.1-4.7 per reorganisation plan.
% All numeric values verified against src/config/hparams.yaml and the
% actual Phase 3 dataset (results/phase3/master_summary.csv), not
% carried over from the Phase 2 draft.
...
enumerate $Q$-values for each action combination and are known to exhibit
instability in long-horizon environments with stochastic
dynamics~\citep{mnih2015humanlevel}. Proximal Policy Optimisation (PPO)
optimises a parameterised policy directly using a clipped surrogate
objective that constrains update magnitude~\citep{schulman2017proximal},
providing the training stability required here. PPO has demonstrated
robust performance in long-horizon control problems and energy-management
...
diesel stockout or battery deep-discharge causes immediate, irreversible
service loss. \texttt{MaskablePPO} from \texttt{sb3-contrib} instead sets
infeasible action logits to $-\infty$ before the softmax, making the
probability of any infeasible action exactly zero by construction. This
converts the constraint set $\mathcal{C}(s_t)$ into a hard structural
property of the policy distribution and eliminates penalty-coefficient
tuning entirely.
...
reward signal. Whether that benefit justifies the added learning
complexity is the central empirical question (Hypothesis~H1, tested via
the comparator policies of Section~\ref{sec:comparison_policies}). All
...
($p=1/336$, mean 336\,h). Normal and Delayed are used during training;
Monsoon and Extreme are evaluation-only out-of-distribution stress tests.
...
simulator calibration. The final evaluation uses four representative
logistics scenarios that span the range from routine operation to severe
delivery disruption while avoiding redundant intermediate cases. The
selected scenarios provide progressively increasing logistics uncertainty
while maintaining clear operational interpretation; additional
intermediate settings evaluated during development exhibited similar
behavioural trends and were therefore omitted from the final experimental
protocol to avoid redundant comparisons. Together, these four scenarios
provide representative operating regimes for routine through severely
disrupted logistics while maintaining a manageable and interpretable
experimental design.
...
Lagrangian update was implemented; masking alone drove hard violations to
zero across all evaluated conditions (verified in
Chapter~\ref{chap:results}).
...
\draw[->] (reward) -- (ppo);
\draw[->] (ppo)    -- (policy);
\draw[->] (policy.north) |- ++(0,0.9) -| (order.north);
\end{tikzpicture}
...
three (42, 123, 777) were retained from an earlier pilot phase and the
...
\begin{itemize}
\item \textbf{$\lambda = 100$} (unmet-load penalty) is set roughly 12
times larger than the diesel-cost weight $\alpha$, reflecting that one
...
tariff structures, but is not free; a intermediate weight between zero
...
Chapter~\ref{chap:results}, where $\mu$ varying by an order of magnitude
either side of its default value produces the smallest change in EENS of
any coefficient tested --- confirming that masking, not the penalty term,
is what actually drives constraint satisfaction.
\end{itemize}

These are engineering judgements about relative priority, not values
fitted to data; Chapter~\ref{chap:results} reports a systematic
sensitivity analysis in which each coefficient is independently varied
across a wide range (including a 20-fold range for $\lambda$) to confirm
that the primary comparison in this thesis is not an artefact of this
...
this is a documented, deliberate advantage given to the baseline. Because
the 24-hour horizon cannot see a delivery whose expected arrival lies
beyond it, MPC's expected-delivery estimate is undefined under the
Extreme scenario (mean lead time 336\,h, far beyond the horizon) --- the
central structural limitation this comparison is designed to expose. A
...
\textbf{Oracle-MPC} is identical to MPC except that its forecast is the
\emph{actual} realised future load and solar trace (perfect information)
rather than a persistence estimate, and its planning horizon extends to
the full 360-step evaluation episode rather than 24 hours. It is not a
...
unchanged. This isolates the contribution of explicit inventory and
ordering-history awareness in the state.

\textbf{A6} isolates the contribution of the learned ordering policy. In
...
\begin{tabular}{p{1.6cm}p{2.6cm}p{2.6cm}p{2.4cm}p{3.6cm}}
\toprule
\textbf{Label} & \textbf{Ordering} & \textbf{Dispatch} & \textbf{Forecast} & \textbf{Primary question} \\
\midrule
B0      & Fixed threshold (uncalibrated) & Threshold rule & --- & Baseline anchor \\
B1      & $(s,S)$ rule          & Threshold rule       & --- & Value of intelligent ordering \\
MPC     & $(s,S)$ rule          & MILP, $H{=}24$       & Persistence & Does optimal-but-myopic dispatch close the gap? \\
Oracle-MPC & $(s,S)$ rule       & MILP, $H{=}360$      & Perfect & Upper bound given perfect information \\
TrackB  & $(s,S)$ rule          & Learned (PPO)        & --- & Joint vs.\ hierarchical (H1, confounded) \\
A6      & $(s,S)$ rule          & Learned (PPO, Discrete(2)) & --- & Joint vs.\ hierarchical (H1, cleaner) \\
A7      & Learned (joint)       & Learned (PPO, no mask) & --- & Action masking (H2) \\
A5      & Learned (joint, no inv.\ obs.) & Learned (PPO) & --- & Inventory observability (H3) \\
RLInv   & Learned (joint)       & Learned (PPO)        & --- & Proposed system \\
\bottomrule
\end{tabular}
\end{table}
...
Secondary metrics    & Diesel consumption, uptime, stockout events, orders placed \\
...
are reported in Chapter~\ref{chap:results}. Although the configuration
file retains an earlier default of 30 episodes per seed, all reported
...
Lead-time model
  & Lognormal($\mu$\,=\,2.5\,days normal; 5.0\,days delayed), single
    scalar $\Pipe_t$
  & Geometric distribution across four scenarios (Normal 24\,h, Delayed
    48\,h, Monsoon 72\,h, Extreme 336\,h); flag\,+\,quantity pair.
    Lognormal is evaluated separately as a distributional-robustness
    check in Chapter~\ref{chap:results}, not as the primary training
    distribution. \\[4pt]
Discount factor
...
assumptions and scope boundaries, consolidated here though otherwise
stated individually alongside the relevant formulation and
implementation details (Chapter~\ref{chap:problem},
Table~\ref{tab:deviations} above).
...
state a two-scalar quantity (Section~\ref{sec:telecom_env}) rather than
