...
The selected coefficients encode engineering design preferences regarding
the relative importance of the operational objectives. Their values are
guided by the engineering rationale discussed below and by prior work
\citep{hu2024performance}, while their influence is evaluated through the
reward-sensitivity analysis presented in
Chapter~\ref{chap:results}
reports a systematic sensitivity analysis in which each coefficient is
independently varied across a wide range (including a 20-fold range for
$\lambda$) to confirm that the primary comparison in this thesis is not
an artefact of this particular weighting.
