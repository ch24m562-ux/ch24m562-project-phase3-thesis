...
constraints \citep{camacho2007model}. Within the telecom-tower literature
reviewed above, Ma~\&~Pan~(2025) directly compare their RL controller
against an MPC baseline with imperfect forecasts, finding RL preferable
under their dataset (Section~\ref{sec:ma-pan}); however, no surveyed work
examines MPC specifically as a lens for isolating whether an RL
controller's advantage stems from ordering decisions, dispatch
optimisation, or forecast quality. This distinction motivates the
inclusion of both a practical and a perfect-information MPC comparator in
this thesis (Chapter~\ref{chap:methodology}).
...
\begin{itemize}
    ...
    dispatch and ordering outperforms both classical hierarchical
    decomposition ($(s,S)$ ordering with separate RL dispatch) and
