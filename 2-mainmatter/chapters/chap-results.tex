...
trade-off rather than an inflated or unfair comparison. This pattern
holds scenario-by-scenario, not only in aggregate, and is accompanied by
a consistently larger inventory buffer that helps explain \emph{why}
RLInv avoids energy shortages (Table~\ref{tab:secondary_metrics}). No
policy exhausted its diesel tank during evaluation under any scenario
shown.
...
\textbf{Orders} & \textbf{Mean inv.} & \textbf{Min inv.} \\
 & & (kWh) & (kWh) & /episode & (\%) & (\%) \\
\midrule
...
TrackB's low diesel consumption under Monsoon and Extreme (48.4 and
70.8\,kWh) should not be read as efficiency: its relatively high
inventory in these scenarios reflects fuel sitting unused rather than
converted into served energy, consistent with the under-dispatch already
identified above. TrackB and A6 report identical values under Normal
because both share the same $(s,S)$ ordering rule and the dispatch
policy --- their one point of difference --- has little effect when
logistics are unstressed; the two diverge sharply once stress increases.
...
sites and evaluation seeds (Figure~\ref{fig:lead_sensitivity}).
